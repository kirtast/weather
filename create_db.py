...
conn=sqlite3.connect('DB_maintenance.sqlite')
cur=conn.cursor()
cur.executescript('''
DROP TABLE IF EXISTS Priority;
DROP TABLE IF EXISTS Operator;
DROP TABLE IF EXISTS Null_essay;
DROP TABLE IF EXISTS Project;
DROP TABLE IF EXISTS State;
DROP TABLE IF EXISTS Device;
DROP TABLE IF EXISTS SerialNumber;
DROP TABLE IF EXISTS Category;
DROP TABLE IF EXISTS CodError;
'''
)
cur.executescript('''
CREATE TABLE Priority(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,prio TEXT UNIQUE);
CREATE TABLE Operator(id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT UNIQUE,name TEXT UNIQUE);
